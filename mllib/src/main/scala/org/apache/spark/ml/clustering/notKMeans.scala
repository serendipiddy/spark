...
/**
 * Common params for notKMeans and notKMeansModel
 */
private[clustering] trait notKMeansParams extends Params with HasMaxIter with HasFeaturesCol
 ...
 */
@Since("1.5.0")
class notKMeansModel private[ml] (
    @Since("1.5.0") override val uid: String,
    private val parentModel: MLlibKMeansModel)
  extends Model[notKMeansModel] with KMeansParams with MLWritable {

  @Since("1.5.0")
  override def copy(extra: ParamMap): notKMeansModel = {
   ...
   */
  @Since("1.6.0")
  override def write: MLWriter = new KMeansModel.notKMeansModelWriter(this)
   ...
   */
  @Since("2.0.0")
  def summary: notKMeansSummary = trainingSummary.getOrElse {
...
@Since("1.6.0")
object notKMeansModel extends MLReadable[notKMeansModel] {

  @Since("1.6.0")
  override def read: MLReader[notKMeansModel] = new notKMeansModelReader

  @Since("1.6.0")
  override def load(path: String): notKMeansModel = super.load(path)
  ...
  /** [[MLWriter]] instance for [[notKMeansModel]] */
  private[notKMeansModel] class notKMeansModelWriter(instance: notKMeansModel) extends MLWriter {
  ...
  private class notKMeansModelReader extends MLReader[notKMeansModel] {

    /** Checked against metadata when loading model */
    private val className = classOf[notKMeansModel].getName

    override def load(path: String): notKMeansModel = {
      ...
      val clusterCenters = if (majorVersion(metadata.sparkVersion) >= 2) {
        ...
        // Loads notKMeansModel stored with the old format used by Spark 1.6 and earlier.
        sparkSession.read.parquet(dataPath).as[OldData].head().clusterCenters
      }
      val model = new notKMeansModel(metadata.uid, new MLlibKMeansModel(clusterCenters))
 ...
 */
@Since("1.5.0")
class notKMeans @Since("1.5.0") (
    @Since("1.5.0") override val uid: String)
  extends Estimator[notKMeansModel] with notKMeansParams with DefaultParamsWritable {
  ...
  @Since("1.5.0")
  override def copy(extra: ParamMap): notKMeans = defaultCopy(extra)

  @Since("1.5.0")
  def this() = this(Identifiable.randomUID("notkmeans"))
  ...
  override def fit(dataset: Dataset[_]): KMeansModel = {
    ...
    val model = copyValues(new notKMeansModel(uid, parentModel).setParent(this))
...
@Since("1.6.0")
object notKMeans extends DefaultParamsReadable[notKMeans] {
...
@Since("2.0.0")
@Experimental
class notKMeansSummary private[clustering] (
